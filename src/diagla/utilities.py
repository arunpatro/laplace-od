...
def get_eigenvalues(factors: List[Tensor], verbose: bool = False) -> Tensor:
    ...
    for layer, factor in enumerate(factors):
        ...
        if len(factor) == 2:
            ...
            eigenvalues = torch.cat(
                [eigenvalues, torch.ger(xxt_eigvals, ggt_eigvals).contiguous().view(-1)]
            )
        else:
...
def linear_interpolation(min_val: float, max_val: float, data: array) -> array:
    ...
    """
    return (max_val - min_val) * (data - np.min(data)) / (
...
def binned_kl_distance(
    dist1: array,
    dist2: array,
    smooth: float = 1e-7,
    bins: array = np.logspace(-7, 1, num=200),
) -> float:
...
def negative_log_likelihood(probabilities: array, labels: array) -> float:
    ...
    """
    return -np.mean(
        np.log(probabilities[np.arange(probabilities.shape[0]), labels] + 1e-12)
    )
...
def predictive_entropy(probabilities: array, mean: bool = False) -> Union[array, float]:
...
def kron(a: Tensor, b: Tensor) -> Tensor:
    ...
    """
    return (
        torch.einsum("ab,cd->acbd", [a, b])
        .contiguous()
        .view(a.size(0) * b.size(0), a.size(1) * b.size(1))
    )
...
def setup(required: bool = True, seed: Union[int, None] = 42):
    ...
    parser.add_argument(
        ...
        default=device,
        type=type(device),
        help="Computation device: GPU or CPU",
    )
    parser.add_argument(
        "--torch_dir",
        default=torch_dir,
    ...
    parser.add_argument(
        ...
        default="torch",
        type=str,
        help="GPU/PyTorch or CPU computation (default: Torch)",
    )
    parser.add_argument(
        "--parallel", action="store_true", help="Use data parallelism (default: off)"
    )
    parser.add_argument(
        "--ram",
        default=psutil.virtual_memory()[1] / 1023**3,
        help="Amount of available system memory on process start.",
    )
    parser.add_argument(
        "--cpus", default=cpus, type=int, help="Number of CPUs (default: Auto)"
    )
    parser.add_argument(
        "--workers",
        default=cpus - 1 if cpus < 6 else cpus - 2,
        type=int,
        help="Data loading workers (default: cpus - 1)",
    )
    parser.add_argument(
        "--prefix", default="", type=str, help="Filename prefix (default: None)"
    )
    parser.add_argument(
        "--suffix", default="", type=str, help="Filename suffix (default: None)"
    )
    ...
    parser.add_argument(
        "--epochs", default=1, type=int, help="Number of (training) epochs (default: 1)"
    )
    parser.add_argument(
        "--lr",
        default=1e-3,
        type=float,
        help="Learning rate in SGD training (default: 1e-3)",
    )
    parser.add_argument(
        "--momentum",
        default=0.9,
        type=float,
        help="Momentum in SGD training (default: 0.9)",
    )
    parser.add_argument(
        "--l2",
        default=0,
        type=float,
        help="L2-norm regularization strength (default: 0)",
    )
    parser.add_argument(
        "--optimizer",
        default="random",
    ...
    parser.add_argument(
        "--estimator", default="kfac", type=str, help="Fisher estimator (default: kfac)"
    )
    parser.add_argument(
        "--samples",
        default=30,
        type=int,
        help="Number of posterior weight samples (default: 30)",
    )
    parser.add_argument(
        "--calls",
        default=50,
        type=int,
        help="Number of hyperparameter search calls (default: 50)",
    )
    parser.add_argument(
        "--boundaries",
        action="store_true",
    ...
    parser.add_argument(
        "--exp_id", default=-1, type=str, help="Experiment ID (default: -1)"
    )
    parser.add_argument(
        "--layer",
        action="store_true",
        help="If layer-wise damping should be used. (default: off)",
    )
    parser.add_argument(
        "--pre_scale",
        default=1,
    ...
    parser.add_argument(
        "--augment",
        action="store_true",
        help="Whether to use data augmentation (default: off)",
    )
    parser.add_argument(
        "--norm",
        default=-1,
    ...
    parser.add_argument(
        "--epsilon", default=0, type=float, help="Step size for FGSM (default: 0)"
    )
    parser.add_argument(
        "--rank",
        default=100,
        type=int,
        help="Rank for information form sparsification (default: 100)",
    )
    ...
    parser.add_argument(
        "--stats",
        action="store_true",
        help="Whether to compute running statistics (default: off)",
    )
    parser.add_argument(
        "--calibration",
        action="store_true",
        help="Make calibration plots (default: off)",
    )
    parser.add_argument(
        "--ood",
        action="store_true",
        help="Run ood evaluation/make ood plots (default: off)",
    )
    parser.add_argument(
        "--fgsm",
        action="store_true",
        help="Run FGSM evaluation/make fgsm plots (default: off)",
    )
    parser.add_argument(
        "--loss1d",
        action="store_true",
        help="Evaluate 1D loss data/make 1D loss plots (default: off)",
    )
    parser.add_argument(
        "--loss2d",
        action="store_true",
        help="Evaluate 2D loss data/make 2D loss plots (default: off)",
    )
    parser.add_argument(
        "--ecdf",
        action="store_true",
        help="Plot inverse ECDF vs. predictive entropy (default: off)",
    )
    parser.add_argument(
        "--entropy",
        action="store_true",
        help="Plot predictive entropy histogram (default: off)",
    )
    parser.add_argument(
        "--summary", action="store_true", help="Print a model summary (default: off)"
    )
    parser.add_argument(
        "--eigvals",
        action="store_true",
        help="Plot eigenvalue histogram (default: off)",
    )
    parser.add_argument(
        "--hyper",
        action="store_true",
        help="Plot hyperparameter optimization results (default: off)",
    )
    parser.add_argument(
        "--networks", action="store_true", help="Plot network calibration comparison"
    )
    parser.add_argument(
        "--landscapes", action="store_true", help="Plot loss and accuracy landscapes"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Give verbose output during execution. (default: off)",
    )
    parser.add_argument(
        "--seed", default=seed, type=int, help="Random seed (default: 42)."
    )

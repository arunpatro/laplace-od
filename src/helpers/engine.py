...
def train_one_epoch(
    model, optimizer, data_loader, device, epoch, print_freq, scaler=None, wandbrun=None
):
    model.train()
...
def evaluate(model, data_loader, device, wandbrun=None, coco_evaluator=None):
    ...
    for images, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        for target, output in zip(targets, outputs):
            if isinstance(target["image_id"], int):
                res[target["image_id"]] = output
            elif isinstance(target["image_id"], torch.Tensor):
